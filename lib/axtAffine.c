...
*/
{
    ...
    while(1)
    {
        ...
        if (nrows > (lw-r)) {
            nrows=lw-r;   /* may get less than k on last set */
        }
        kmax = ki+nrows-1;
    ...
    */
    ...
    while(1)
    {
        while(1)
        {
            ...
            if ((r==0) && (c==0)) {
                break;   /* hit origin, done */
            }
            if (r<rr) {
                break;   /* ran out of targ seq, backup and reload */
            }
            if (dir=='x') {
                errAbort("unexpected error backtracing");   /* x only at origin */
            }
            if (dir=='s') {
                break;   /* hit start, local ali */
            }
            if (dir=='m') /* match */
            {
